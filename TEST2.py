...
def create_ssn_net(img_height, img_width,
    ...
    if phase == 'TRAIN':
        n.img, n.spixel_init, n.feat_spixel_init, n.label, n.problabel = \
            L.Python(python_param=dict(module="input_patch_data_layer", layer="InputRead",
                                       param_str="TRAIN_1000000_" + str(num_spixels)),
                     include=dict(phase=0),
                     ntop=5)

        # the data of del 不知道对不对 ImageLabelmapData 已经不怎么用
        n.data, n.seg_label, n.sp_label = L.ImageLabelmapData( include = dict( phase = TRAIN ), transform_param=
        dict(mirror=False, mean_value = 104.00699 ), image_data_param=dict(
            root_folder="../../data/BSDS500/", source="../../data/BSDS500/train_gpu.lst", batch_size=1,
            shuffle=True, new_height=0, new_width=0))

    elif phase == 'TEST':
        n.img, n.spixel_init, n.feat_spixel_init, n.label, n.problabel = \
            L.Python(python_param=dict(module="input_patch_data_layer", layer="InputRead",
                                       param_str="VAL_10_" + str(num_spixels)),
    ...
    if phase == 'TRAIN' or phase == 'TEST':
        ...
        # the loss of del
        # superpixel_pooling
        # n.superpixel_pooling_out= L.SuperpixelPooling(n.conv_dsp)
